...
- [Compiling to a backend](guide/compiling-to-a-backend.md) — compile the canonical `.daedalus/` definition into your configured backend's native format (for Claude Code: `.claude/agents/`, `.claude/commands/` from your prompts, and `settings.json`), with validate-first safety, a dry-run preview, and clear exit codes: `daedalus build` (alias `sync`).
